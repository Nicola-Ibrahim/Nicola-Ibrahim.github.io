...
- Table of contents (optional — Phase 2).
- Prev / Next post navigation.

#### `/contact` — Contact / Hire Me
- Contact form: name, email, message fields.
- ⚠️ **GitHub Pages has no server** — form must use a static form provider.
  - Recommended: **Formspree** (free tier, no backend needed) or **Netlify Forms** (if you ever migrate).
  - The `<form>` POSTs to Formspree's endpoint. No API route needed.
...
**Project entry (`/content/projects.ts`):**
```ts
interface Project {
  slug: string;
  title: string;
...
- Active link highlighting using `usePathname()` in a `'## 3. Implementation Plan — Phases
...
> **Next step:** Review this plan, answer the Open Questions in Section 4, and share your design tokens. Once approved, I'll split this into `constitution.md` + `spec.md` ready to drop into your repo.
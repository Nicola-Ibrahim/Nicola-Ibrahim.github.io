  ...
  return (
    <div 
      className={`bg-slate-50/50 dark:bg-slate-900/40 backdrop-blur-sm border rounded-2xl transition-all duration-500 ease-in-out overflow-hidden hover:bg-slate-100/50 dark:hover:bg-slate-900/60 ${
        isExpanded ? 'border-teal-500/50 shadow-lg shadow-teal-500/2 dark:shadow-teal-500/2 ring-1 ring-teal-500/10' : 'border-slate-200 dark:border-white/5'
      }`}
    >
      {/* Accordion Trigger */}
      <button 
      ...
      >
        <div className="flex-1">
          <h4 className={`text-xl font-bold transition-colors ${isExpanded ? 'text-teal-600 dark:text-teal-400' : 'text-slate-900 dark:text-slate-100 group-hover:text-teal-500 dark:group-hover:text-teal-300'}`}>
            {task.title}
          </h4>
          <p className="text-slate-700 dark:text-slate-500 mt-2 line-clamp-1 font-medium transition-colors group-hover:text-slate-900 dark:group-hover:text-slate-400">
            {task.shortDesc}
          </p>
        </div>
        <div className={`p-3 rounded-full border transition-all duration-500 ${isExpanded ? 'rotate-180 bg-teal-600 text-white border-teal-500 shadow-md shadow-teal-500/10' : 'bg-slate-200/50 dark:bg-white/5 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/5 group-hover:border-slate-300 dark:group-hover:border-white/10'}`}>
      ...
      >
        <div className="px-8 pb-10 pt-4 border-t border-slate-100 dark:border-white/5 flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex-1 min-w-0">
            <h5 className="text-[10px] font-black text-teal-600 dark:text-teal-400 tracking-[0.3em] uppercase mb-6 flex items-center gap-3">
              <span className="w-6 h-px bg-teal-600/30 dark:bg-teal-400/30"></span>
              Content & Theory
            </h5>
            ...
            {task.prompt && (
              <div className="mt-8 mb-4 bg-slate-100 dark:bg-black rounded-2xl overflow-hidden border border-slate-200 dark:border-white/5 shadow-inner transition-colors">
                <div className="flex items-center justify-between px-6 py-4 bg-slate-200/50 dark:bg-white/5 border-b border-slate-200 dark:border-white/5">
                  <span className="text-[10px] font-black tracking-[0.2em] text-teal-600 dark:text-teal-400 flex items-center gap-2 uppercase">
                    <Sparkles className="w-4 h-4"/> System Prompt
                  </span>
                  <button 
                    onClick={() => onCopy(task.prompt!, task.id)}
                    className="flex items-center gap-2 px-4 py-2 bg-teal-600/10 hover:bg-teal-600/20 text-teal-600 dark:text-teal-400 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all border border-teal-500/10"
                  >
                    {copiedId === task.id ? (
                ...
                <div className="p-8 text-slate-800 dark:text-slate-400 font-mono text-sm whitespace-pre-wrap leading-relaxed overflow-x-auto selection:bg-teal-500/40">
                  {task.prompt}
                </div>
              </div>
            )}
          ...
          {task.image && (
            <div className="w-full lg:w-5/12 shrink-0">
              <div className="aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10 relative group/img">
                <div className="absolute inset-0 bg-teal-600/20 group-hover/img:bg-transparent transition-colors duration-700 z-10"></div>
